...
from ...types.builder import onboard_create_params
from ...types.builder.onboard_create_response import OnboardCreateResponse
...
class OnboardResource(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        """Sets up an end-user for trading on a specific exchange.

        Managed mode creates
        wallet + exchange credentials. Self mode stores user-provided credentials.

        Args:
          customer_id: Parsec customer ID of the end-user to onboard.
          ...
          chain_id: Chain ID for Safe wallet creation.
          ...
          eoa_address: External wallet address (required when wallet_type is "safe").

          private_key: Kalshi RSA private key in PEM format (self mode).

          wallet_type: Wallet type for managed mode ("eoa" or "safe").
