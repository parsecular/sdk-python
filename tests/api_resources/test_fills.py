...
class TestFills:
    ...
    def test_method_list(self, client: ParsecAPI) -> None:
        ...
        assert_matches_type(FillListResponse, fill, path=["response"])
    ...
    def test_raw_response_list(self, client: ParsecAPI) -> None:
        response = client.fills.with_raw_response.list(
            exchange="exchange",
        )
    ...
    def test_streaming_response_list(self, client: ParsecAPI) -> None:
        with client.fills.with_streaming_response.list(
            exchange="exchange",
        ) as response:
...
class TestAsyncFills:
    ...
    async def test_raw_response_list(self, async_client: AsyncParsecAPI) -> None:
        response = await async_client.fills.with_raw_response.list(
            exchange="exchange",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        fill = await response.parse()
        assert_matches_type(FillListResponse, fill, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncParsecAPI) -> None:
        async with async_client.fills.with_streaming_response.list(
            exchange="exchange",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
